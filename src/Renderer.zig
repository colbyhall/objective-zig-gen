...
pub fn run(options: Options) Error!void {
    ...
    try self.render("// Objective C Runtime in zig. \nconst objc = @import(\"objc.zig\");\n", .{});
    for (framework.dependencies) |dependency| {
        const dep = self.frameworks.get(dependency).?;
        try self.render("// Framework dependency {s}. \nconst {s} = @import(\"{s}.zig\");\n", .{ dep.name, dep.output_file, dep.output_file });
    }
    try self.render("\n", .{});

    if (self.analyzer.type_table.protocols.keys().len > 0) {
        try self.render("// ------------------- PROTOCOLS ------------------- \n", .{});
    ...
    if (self.analyzer.type_table.interfaces.keys().len > 0) {
        try self.render("// ------------------- INTERFACES ------------------- \n", .{});
